...
for (var i = 0; i < browsername.length; i++) {
    if (AgentInfo.indexOf(browsername[i]) >= 0) {
        browser = browsername[i];
        break;
    }
}
...
function add_css(str_css) {
    ...
    if (cssnow !== null) {
        try {
            stlnow.removeChild(cssnow);
        } catch (e) {
        }
    }
    try {
        var mystl = document.createStyleSheet();//ie
        mystl.id = "mystyle";
        mystl.cssText = str_css;
    }
    catch (e) {
        var mystl = document.createElement("style");
        mystl.id = "mystyle";
        mystl.type = "text/css";
        mystl.textContent = str_css;
...
function openwindow(d) {
    d = d.indexOf("http") < 0 ? "http://" + d : d;
    window.open(d);
}

function closewindow() {
    if (window != window.parent) {
        window.parent.hide('s_select')
    }
    else {
        window.opener = null;
        window.open('', '_self');
...
function mouse_S() {
    ...
    for (var i = 0; i < objs.length; i++) {
        objs[i].addEventListener("mousewheel", function (e) {
            if (this.scrollLeft > 0 && this.scrollLeft < (this.scrollWidth - this.offsetWidth)) {
...
function show() {
    for (var i = 0; i < arguments.length; i++) {
        var ob = (typeof arguments[i] != "object") ? document.getElementById(arguments[i]) : arguments[i];
        ob.classList.remove('none');
    }
}
...
function togg(d, t) {
    ...
    if (t) {
        event.srcElement.innerHTML = ob.classList.contains("none") ? t[0] : t[1];
    }
...
function no_detail(d, t) {//详情全展开或隐藏
    ...
    t = t || event.srcElement.title;
    event.srcElement.title = t * -1;
    for (var i = 0, ln = p.length; i < ln; i++) {
        p[i].open = t == 1 ? true : false;
    }
}
...
function showfulltxt() {
    event.srcElement.classList.toggle("ellips5");
}

function R_show(d, s) {
    if (s || !d) {
        var eob = event.srcElement, eobs = eob.parentNode.children, ln = eobs.length;
        for (var i = 0; i < ln; i++) {
            eobs[i].classList.toggle("bgca5", false);
        }
        eob.classList.toggle("bgca5", true)
    }
    ;
    if (d) {
        var eob = (typeof d != "object") ? document.getElementById(d) : d, eobs = eob.parentNode.children,
            ln = eobs.length;
        for (var i = 0; i < ln; i++) {
            eobs[i].classList.toggle('none', true)
        }
...
function fadein(d) {
    var c_n = 0, v = 0.1, d = d ? d : document.body;
    show(d);
    fadei(d);
...
function fadeout(d) {
    var c_n = 0, v = 0.9, d = d ? d : document.body;
    fadeo(d);
...
function clearval() {
    if (arguments.length == 0) {
        ...
        for (var z in arguments) {
            document.getElementById(arguments[z]).value = "";
        }
    }
}
...
function A_C(d, c) {
    if (d.className.indexOf(c) < 0) {
        d.className = d.className + " " + c;
    } else {
        d.className = d.className.replace(eval("/" + c + "/i"), "")
    }
}
...
function tog_fun() {
    ...
    if (n >= 0 && tex.length == funs.length) {
        n = (n + 1) % tex.length;
        eobj.parentNode.parentNode.title = eobj.innerHTML = tex[n];
        eval(funs[n] + "()");
    }
}
...
function set_title(d, v) {
    document.getElementById(d).title = v
}
...
function removeobj(d, n) {
    if (n && n == "name") {
        var obs = document.getElementsByName(d), ln = obs.length;
        for (var i = 0; i < ln; i++) {
            obs[i].parentNode.removeChild(obs[i])
        }
    }
    else {
        ob = d ? ((typeof d != "object") ? document.getElementById(d) : d) : event.srcElement.parentNode;
...
function del_parent(d) {
    if (d) {
        var dobj = document.getElementsByName(d), ln = dobj.length;
        for (var i = 0; i < ln; i++) {
            dobj[i].parentNode.parentNode.removeChild(dobj[i].parentNode)
        }
    }
    else {
...
function slideTo(n, d) {
    var obj = d ? document.getElementById(d) : event.srcElement;
    console.log(obj);
...
function P_V(d) {
    ...
    if (last_obj.tagName == "INPUT") {
        last_obj.value = vobj.innerHTML || vobj.value || "";
    }
    else {
        last_obj.innerHTML = vobj.innerHTML || vobj.value || "";
    }
    last_obj.title = vobj.title || "";
    if (d) {
        localStorage.setItem(d, vobj.title)
    }
    ;
    hide("s_select");
...
var cYear, cMonth, cDay, monString = ["正月", "二月", "三月", "四月", "五月", "六月", "七月", "八月", "九月", "十月", "冬月", "腊月"],
    dayString = ["初一", "初二", "初三", "初四", "初五", "初六", "初七", "初八", "初九", "初十", "十一", "十二", "十三", "十四", "十五", "十六", "十七", "十八", "十九", "廿十", "廿一", "廿二", "廿三", "廿四", "廿五", "廿六", "廿七", "廿八", "廿九", "三十"],
    date_sli = ['year', 'month', 'day', 'hour', 'minute', 'second', 'week'],
    madd = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334],
    astroString = ["魔蝎座", "水瓶座", "双鱼座", "白羊座", "金牛座", "双子座", "巨蟹座", "狮子座", "处女座", "天秤座", "天蝎座", "射手座"],
    astroN = ["21", "20", "21", "21", "22", "22", "23", "24", "24", "24", "23", "22"];
...
//获得默认时间
    if (browser == "witknow" && flag < 5) {
        postmessage({
            functionname: "gettimesel",
            witparams: {mgc: n, showtype: gs, initime: last_val},
            callback: "p_time"
        });
    } else {
        c_date1(n, gs, me);
...
function c_date1(n, gs, me) {
    ...
    set_ymdhm(date);
    var cont = '<div class="W11 H11 bgca" onClick="hide(&quot;s_select&quot;)">' +
        '</div>' +
        '<div id="sle" class="fix bottom0 W11 H35M bgc102 PB1">' +
        '<div id="change_table" class="none index99 fix bottom0 FL W11 H35M bgc102 AC" >' +
        '</div>' +
        '<div class="FL MLT A51 H4M F3 AC bgc36" onclick="change_date_type()">' + date_show_type + '</br>阳历</div>';
    cont += '<div id="inputmess" class="FL MLT A53 H AC bgc9" >' +
        '</div><div id="tt" class="FR MRT A51 H AC bgc16 color1" onclick="P_V(&quot;inputdate&quot;)">确认</div>' +
        '<div id="scr_pp" class="FL ML A11 H28M" >' +
        '<div class="no_event index999 absolute bottom13 W11 H bgc59 alpha2">' +
        '</div>';
    for (var i = 0; i < nl; i++) {
        cont += '<div id="' + date_sli[n.charAt(i)] + '" name="s" class="ofa FL MT H11 W' + nl + '1 bgc' + (1 + i) + '" ></div>'
    }
...
function creat_date(n, gs, me) {
    last_obj = event.srcElement;
    last_val = event.srcElement.title || ((me == 1 && localStorage.inputdate) ? localStorage.inputdate : new Date().Format('yyyy-MM-dd hh:mm:ss'));
...
function C_scrolli(k) {
    ...
    if (ye == 2) {
        var w = (new Date(year, month - 1, 1)).getDay();
...
function turn() {
    ...
    if ((ppobj.id == "year" || ppobj.id == "month") && outtype.indexOf(2) >= 0) {
        C_scrolli(outtype.indexOf(2))
    } else {
        displaydate("inputmess")
    }
}
...
function alladjust() {
    ...
    setTimeout(function () {
        ob.childNodes[0].childNodes[Math.round(0.25 * ob.scrollTop / M) + 3].click();
    }, 500);
}
...
function displaydate(d, da, t) {
    ...
    if (date_show_type == "农历" && outtype.indexOf('012') >= 0) {
        TOCNDate(date, d)
    }
    else {
        for (var i = 0; i < outtype.length; i++) {
            var dxt = eval(date_sli[outtype[i]]);
            outtext += (dxt > 9 ? dxt : "0" + dxt) + (date_geshi[i] ? date_geshi[i] : "");
        }
        ;
        if (date_geshi.indexOf('周') >= 0) {
            outtext += "周" + "日一二三四五六".charAt(date.getDay())
        }
        ;
        if (tob.tagName == "INPUT") {
            tob.value = outtext;
        } else {
            tob.innerHTML = outtext;
        }
    }
}
...
function TOCNDate(D, d) {
    e2c(D);
    var tob = document.getElementById(d),
        tmp = cYear + "甲乙丙丁戊己庚辛壬癸".charAt((cYear - 4) % 10) + "子丑寅卯辰巳午未申酉戌亥".charAt((cYear - 4) % 12) + "(" + "鼠牛虎兔龙蛇马羊猴鸡狗猪".charAt((cYear - 4) % 12) + ")年 " + monString[cMonth - 1] + dayString[cDay - 1];
    if (tob.tagName == "INPUT") {
        tob.value = tmp
    } else {
        tob.innerHTML = tmp
...
function pas_zodi() {
    if (document.getElementById('zodiac')) {
        document.getElementById('zodiac').value = document.getElementById('birth_day').value.split("(")[1].split(")")[0];
    }
    ;
    if (document.getElementById('astro')) {
        document.getElementById('astro').value = astroString[(month - 1 + Math.floor(day / astroN[month - 1])) % 12];
    }
}
...
function returndate(t, da) {
    var outtext = "";
    if (da) {
        date = new Date(da.rtrim(".0").replace(/\-/g, "/"))
    } else {
        date = new Date()
    }
    ;
...
function DateAdd0(i, n, date) {//日期加减
    ...
    if (!date) {
        ...
        date = new Date(date.rtrim(".0").replace(/\-/g, "/"))
    }
    ;
    n = n >= 0 ? n : 1;
    console.log(n);
...
function DateDif(i, date1, date0) {//返回两个日期之间的差值
    ...
    if (!date1) {
        ...
        date1 = new Date(date1.rtrim(".0").replace(/\-/g, "/"))
    }
    ;
    if (!date0) {
        date0 = new Date()
    } else {
        date0 = new Date(date0.rtrim(".0").replace(/\-/g, "/"))
    }
    ;
    ...
    if (browser == "witknow") {
        ...
        if (flag < 5) {
            postmessage({
                ...
                witparams: {ileve: nl, areacode: area, iniaddress: old},
                callback: "p_addr"
            });
        } else {
            c_addr1(nl, area);
...
function getarea(j, po) {
    if (scrollable == true) {
        ...
        potitle = po || "";
        uln = j;
        if (event.srcElement.title == 'lasto' && event.srcElement.nextSibling.innerHTML) {
            inpm.title = refo.innerHTML = event.srcElement.parentNode.title
        }
        else if (po !== undefined) {
            inpm.title = refo.innerHTML = po
        }
        ;
        if (j > ulnl) {
            var tex = "";
            for (var i = 1; i <= ulnl; i++) {
