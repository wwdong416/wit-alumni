        ...
        if (m_Error(msg, "获取校友会信息")) {
            ...
            if (p.length > 0) {
                ...
                document.getElementById("i_class").onclick = function () {
                    window.open(location.href.replace('H5/areaclass', 'com/manager') + "&t=" + p[0].type);
                };
                document.getElementById("classLogo").onclick = function () {
                    class_msg(p);
                };
            }
            else {
...
function class_msg(s) {
    ...
    div.innerHTML = '<div class="fix top0 H W11 AC bgc10 ffHT">校友会信息' +
        '    <div  class="FL B4M H4M F2" onclick="mob_hide()">' +
        ...
        '<div class=" MT2 M F4 bold ellips">' + s[0].name + '校友会</div>' +
...
function Re_notice() {
    if (flagCheck === 2) {
        ...
        div.innerHTML = '<div class="top0 H W11 AC ffHT bgc10 ">发布通知' +
            '    <div  class="FL B4M H4M F2" onclick="mob_hide()"> 取消</div>' +
...
function fill_notice(p, page) {
    ...
    p.forEach(function (v) {
        ...
        div.innerHTML = '  <div class="W11 P1M bordBD1">' +
            ...
            '<div class="W11 FL">' +
            '<div class="FL AC"><img class="W32" src="../images/icon/lpl.png" alt=""></div>' +
            '</div>' +
    ...
    _wd.ajax_formdata(url + "/notice/queryByReplay.do", true, para, function (msg) {
        ...
        if (m_Error(msg, "加载回复通知")) {
            ...
            p.forEach(function (v) {
                ...
                div.innerHTML = '<div class="W11 word_break"><span class="active">' + v.editor + ' : </span>' + v.notice + '</div>';
...
function classmatesMember(p) {
    ...
    cont.innerHTML = "";
    cont.setAttribute("data-hash", "classmates_page");
    var div = document.createElement("div");
    div.className = "fix top0 F3  W11 CHmax bgc9 ofa";
    var msgHtml = '<div class="fix index100 top0 H bgc10 W11 AC ffHT ">成员信息' +
        '<div class="FL B4M H4M F2" id="exit_c_page" onclick="mob_hide();"> <img  src=\'../images/icon/back_b.png\' class="FL B4M H4M P1M" /></div>' +
...
function newRecord($o) {
    ...
    if (flagCheck === 2) {
        ...
        div.innerHTML = '  <div class="fix top0 H bgc10 ffHT W11 AC ffHT ">上传档案' +
            '<div class="FL B4M H4M F2" onclick="mob_hide()"> <img  src="../images/icon/back_b.png" class="FL B4M H4M P1M" /></div>' +
                    ...
                    for (var l = 0; l < this.files.length; l++) {
                        ...
                        reader.onloadend = function (e) {
                            ...
                            if (type === "1" || type === "5") {
                                ...
                                div2.innerHTML = '<div class="F3 C11M ellips FL H4M P1M">' + this.name + '</div><div class="FL F2 color8 B6M AR H4M P1M">' + videoSize + '</div><div class="FR B4M H4M"><img src="../images/icon/delete.png" class="P1M FR B4M H4M"  alt=""></div>';
...
function getMorePH_time(page, y, m) {
    ...
    _wd.ajax_formdata(url + "/record/queryByDate.do", true, para, function (msg) {
        if (m_Error(msg, "按时间获取档案")) {
            ...
            if (j.length === 0) {
                if (page === 1) {
                    dy_photo.innerHTML = ' <div class=" colorA MT W11  AC ofh">没有找到此月发布的档案</div>';
                } else {
                ...
                j.forEach(function (v) {
                    ...
                    if (sameTime !== date) {
                        ...
                        ph_title = '  <div class="FL W11 H3M">' +
                            '                    <div class="FL H3M LH2 AL color876 bold W21 ellips">' + v.phone + '</div>' +
                            '                    <div class="FR H3M LH25 AR W21 PR1M F2 colorA LH2 ellips">' + _wd.crtTimeYMD(v.date) + '</div>' +
                            '                </div>' +
                            '                <div class="FL W11 colorA F2">' +
                            '                    <div class="W43 FL AL ellips">' + v.memo + '</div>' +
                            '                    <div class="W41 FR AR PR1M">' + _wd.crtTimeHM(v.date) + '</div>' +
                            '                    <div class="clear"></div>' +
                            '                </div>' +
                    ...
                    if (v.type === 1 || v.type === 5) {
                        ...
                        d.innerHTML =
                            ...
                            '<video class="W11 H16M" controls="controls" src="' + imgHtml + v.src + '"  preload="" >' +
                ...
                j.forEach(function (v) {
                    ...
                    if (sameTime !== date) {
                        ph_title = '  <div class="FL W11 H3M">' +
                            ...
                            '                </div>' +
                            '                <div class="FL W11 colorA F2">' +
                            '                    <div class="W43 FL AL">' + v.memo + '</div>' +
                            '                    <div class="W41 FR AR PR1M">' + _wd.crtTimeHM(v.date) + '</div>' +
                            '                    <div class="clear"></div>' +
                            '                </div>' +
                            '               <div class="FL W11" id="' + list_id + '">' +
                    ...
                    if (v.type === 1 || v.type === 5) {
                        ...
                        d.innerHTML =
                            '<div class=" H16M bgc9 AC ofh">' +
                            '<video class="W11 H16M OFC" controls="controls" src="' + imgHtml + v.src + '"  preload="" >' +
                            ' <source src="' + imgHtml + v.src + '">' +
                            '</video>' +
                            '                    </div>' +
                            '                <div class="clear"></div>';
                    ...
                    switch ($type) {
                        ...
                        case 2:
                            ...
                            div.innerHTML = '<div class="W11 H21M "><div class="W11 AL P1M ellips">' + v.name + '</div>' +
                                '<video class="W11 H21M " controls="controls" src="' + imgurl + v.src + '"  preload="none" >' +
